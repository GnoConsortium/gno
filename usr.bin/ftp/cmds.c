...
{
	getit (argc, argv, 1, "r+wb");
}

void get (int argc, char **argv)
{
	getit (argc, argv, 0, restart_point ? "r+wb" : "wb" );
}
...
char *cmd;
	...
	recvrequest(cmd, argv[2], argv[1], "wb", 0);
}
...
#else
	if (argc <= 1) {
		printf("Shell only not supported under GNO.  Please specify a command also.\n");
		code = -1;
		return;
	}
	if (debug) {
...
{
register int i, len;
static char buf[BUFSIZ];		/* must be >= sizeof(line) */
...
#else
	if (argc == 1) command("SITE UMASK");
	else command("SITE UMASK %s", argv[1]);
#endif
	verbose = oldverbose;
}
...
#else
	if (argc == 1) command("SITE IDLE");
	else command("SITE IDLE %s", argv[1]);
#endif
	verbose = oldverbose;
}
...
#else
	if (argc > 1) command("STAT %s", argv[1]);
	else command("STAT");
#endif
}
